...
class CombineTab:
    ...
    def set_red_channel(self):
        ...
        if file:
            ...
            self.label_red.configure(text=os.path.basename(file), text_color="white")
            self.update_preview()
            Logger().log(f"Red channel input loaded: {os.path.basename(file)}", "INFO")
    ...
    def set_green_channel(self):
        ...
        if file:
            ...
            self.label_green.configure(text=os.path.basename(file), text_color="white")
            self.update_preview()
            Logger().log(f"Green channel input loaded: {os.path.basename(file)}", "INFO")
    ...
    def set_blue_channel(self):
        ...
        if file:
            ...
            self.label_blue.configure(text=os.path.basename(file), text_color="white")
            self.update_preview()
            Logger().log(f"Blue channel input loaded: {os.path.basename(file)}", "INFO")
    ...
    def set_alpha_channel(self):
        ...
        if file:
            ...
            self.label_alpha.configure(text=os.path.basename(file), text_color="white")
            self.update_preview()
            Logger().log(f"Alpha channel input loaded: {os.path.basename(file)}", "INFO")
    ...
    def generate(self):
        try:
            if not self.output_file:
                Logger().log("Please select an output file", "WARNING")
                return
            ...
            if not any(channels_set):
                Logger().log("Please select at least one input channel", "WARNING")
                return
            ...
            if not reference_img:
                Logger().log("No valid input channels found", "ERROR")
                return
            ...
            Logger().log(f"Combined image saved successfully: {os.path.basename(self.output_file)}", "SUCCESS")

        except Exception as e:
            Logger().log(f"Failed to combine channels: {str(e)}", "ERROR")
